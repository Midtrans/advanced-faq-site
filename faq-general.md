...
### Which of the Credit Card acquiring banks need BCA MIGS Pilot testing?{docsify-ignore}
BCA, Maybank, and BRI banks need BCA MIGS Pilot testing for full payment and installment payments.

### I got `Terminal or MID not found` message from log and all card transactions fail. What should I do?{docsify-ignore}
...
### Why Snap pop-up doesn’t work on merchant's mobile app?{docsify-ignore}
...
### Why the Snap payment button is obscured/covered by Android's on-screen navigation bar, when rendered via merchant's app WebView?{docsify-ignore}
...
### Can I revoke or cancel an active *Snap* Token to prevent the customer from proceeding with the transaction?{docsify-ignore}
...
### I want to use Bin API ( /v1/bins/bin_number), but I am getting "Invalid authentication credentials"?{docsify-ignore}
...
### How should I implement offline installment payment?{docsify-ignore}
...
### Is it possible to identify card issuer or brand based on the card number?{docsify-ignore}
...
### I want to use credit card BIN specific promo campaign. How should I implement this?{docsify-ignore}
...
### I want to use payment method specific promo campaign. How should I implement this?{docsify-ignore}
...
### Why did I get alert message that HTTP notification is failure because of 3xx status code or redirect?{docsify-ignore}
...
### Why does Midtrans HTTP notification received on merchant backend/notification handler look "empty"?{docsify-ignore}
...
### The customer transactions are denied because of fraud detection system. If I trust the customer, can I request whitelist?{docsify-ignore}
...
### I don’t want to send real "customer_details" data to Midtrans. Is it possible? Are there any consequences? How can I do it?{docsify-ignore}
...
### While switching system configuration from Midtrans *Sandbox* environment to Midtrans *Production* environment (or switching between different MID/Account), credit card transactions encounter error message "Credit card token is no longer available". What should I do?{docsify-ignore}
...
### What is Pending, Settlement, Expire transaction status for asynchronous payment?{docsify-ignore}
...
### Failure to redirect the customer to Gojek GoPay, Shopee Pay, and other e-wallet payment provider app. What should I do?{docsify-ignore}
...
### Why am I getting notified to update my system version?{docsify-ignore}
...
### My developer encounters `javax.net.ssl.SSLHandshakeException: Received fatal alert: handshake_failure` while trying to connect to Midtrans API URL. What needs to be done?{docsify-ignore}
...
### Does Midtrans provide SSL certificate or root certificate to download?{docsify-ignore}
...
### Does Midtrans provide IP address of the API domain to be whitelisted from Midtrans side (outbound request from Merchant to Midtrans)?{docsify-ignore}
...
### Customer complains the refund is not yet received, but on Midtrans Dashboard the status has already changed to `refund`, can you check?{docsify-ignore}
...
### We have noticed reported that the transaction amount on the Snap pop-up can be tampered/modified/changed from the customer side, is it safe?{docsify-ignore}
...
### Can merchant force *Snap* to always show deeplink or QR for GoPay transaction?{docsify-ignore}
...
### Why is the customer's fund deducted for a GoPay transaction, even when the transaction is recorded as failure or expired on Midtrans Dashboard?{docsify-ignore}
...
### When can I refund GoPay transaction?{docsify-ignore}
...
### My team/my customer tried failure payment scenario on E-Wallet transaction. But transaction status does not update to failure?{docsify-ignore}
...
### How to use Core API `/register card` endpoint from frontend?{docsify-ignore}
...
### I used Mobile SDK but I am unable to get *Snap* Token. How can I resolve it?{docsify-ignore}
...
### How can I disable debug log on Android Mobile SDK?{docsify-ignore}
...
### Can I configure enabled payments list on Android SDK?{docsify-ignore}
...
### How to display specific payment channel via mobile SDK client code?{docsify-ignore}
...
### I am using Android SDK, but the transaction is failing because 3DS is not enabled. What is wrong?{docsify-ignore}
...
### Does Midtrans support Flutter, React Native, or other hybrid/non-native mobile framework?{docsify-ignore}
...
### During 3DS card transaction frontend callback what does `"status_message": "Failed to generate 3D Secure token"` means?{docsify-ignore}
...
### Using 3DS Card transaction flow v1 Core API, how can I ensure that the transaction is 3DS end to end?{docsify-ignore}
...
### There are some missing field or property on the JSON response or notification. It seems the JSON is not consistent. Is this expected?{docsify-ignore}
...
### There are new fields or properties added on the JSON response or notification. It breaks merchant implementation. Is this expected?{docsify-ignore}
Yes, it is expected.
...
### How can I make sure that the card transaction is 3DS on iOS?{docsify-ignore}
...
### On iOS device why GoPay deeplink redirects the customer to different app other than Gojek?{docsify-ignore}
...
### Why am I unable to do other VA transaction on *Snap*/Mobile SDK?{docsify-ignore}
...
### How can I debug network or API call on Midtrans iOS SDK to check for any API validation error?{docsify-ignore}
...
### Can I customize the data printed on the customers Indomaret payment receipt?{docsify-ignore}
...
### Does Midtrans show the trail history of transaction or payment cycle?{docsify-ignore}
...
### Transaction has been paid by the customer, but the status is not updated on my system. What is the issue?{docsify-ignore}
